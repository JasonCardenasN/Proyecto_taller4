...
export async function GET(request: Request) {
  try {
    ...
    const sheetIndexParam = url.searchParams.get('sheetIndex');
    
    const sheetIndex = sheetIndexParam ? parseInt(sheetIndexParam, 10) : 0;
    
    if (sheetIndex < 0 || sheetIndex >= doc.sheetsByIndex.length) {
      return NextResponse.json({ error: "Sheet index out of bounds" }, { status: 404 });
    }
